  ...
  const rmw_qos_profile_t * qos_policies)
{
  ...
  if (!node) {
    ...
    rmw_client = reinterpret_cast<rmw_client_t *>(rmw_allocate(
        sizeof(rmw_client_t)));
    ...
    rmw_ertps_mempool_item_t * memory_node = get_memory(&client_memory);
    if (!memory_node) {
      RMW_SET_ERROR_MSG("Not available memory node");
      goto fail;
    }

    rmw_ertps_client_t * custom_client = reinterpret_cast<rmw_ertps_client_t *>(memory_node->data);
    ...
    custom_client->writer = custom_node->context->domain->createWriter(
      *custom_node->context->participant,
      req_topic_name,
      req_type_name,
      qos_policies->reliability != RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT);

    if (nullptr == custom_client->writer) {
      goto fail;
    }

    custom_client->reader = custom_node->context->domain->createReader(
      *custom_node->context->participant,
      res_topic_name,
      res_type_name,
      qos_policies->reliability != RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT);

    if (nullptr == custom_client->reader) {
      goto fail;
    }

    custom_client->reader->registerCallback(
      generic_callback<rmw_ertps_client_t>,
      custom_client->reader);
